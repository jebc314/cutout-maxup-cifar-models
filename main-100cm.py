...
def main():
    ...
    if use_gpu:
        ...
        model = resnet110_cifar(num_classes=100) # MODIFICATION-using 100
        ...
        if not os.path.exists('result'):
            os.makedirs('result')
        fdir = 'result/resnet110_cifar100-maxup' # MODIFICATION-saving to right name
    ...
    # CIFAR100
        ...
        train_dataset = torchvision.datasets.CIFAR100(
            ...
            transform=transforms.Compose([
                ...
                transforms.ToTensor(),
                normalize,
                Cutout(n_holes=1, length=8) # MODIFICATION-Add cutout transformation
            ]))
        trainloader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=2)
...
def train(trainloader, model, criterion, optimizer, epoch):
    ...
    def cutout(image_list):
      temp_cutout = Cutout(1,8)
      for i, x in enumerate(image_list):
        image_list[i] = temp_cutout(x)
      return image_list
    ...
    for i, (input, target) in enumerate(trainloader):
        ...
        # MODIFICATION-Add MaxUp
        num_classes = 100
        if epoch >= 10:
            with torch.no_grad():
                model.eval()
                sample_size = 5
                noisy_inputs = cutout(input.view(1, -1, 3, 32,
32).repeat(sample_size, 1, 1, 1, 1).view(-1, 3, 32, 32))
                _outputs = model(noisy_inputs)

                inds =  torch.min(F.softmax(_outputs,
dim=1).view(sample_size, -1, num_classes),
dim=0)[1][torch.arange(0, target.size(0)).long().cuda(),
target]
                input = noisy_inputs.view(sample_size, -1, 3072)[inds,
torch.arange(0, inds.size(0)).long().cuda()].view(-1, 3,
32, 32)
                model.train()
...
def adjust_learning_rate(optimizer, epoch, model_type):
    ...
    if model_type == 1: # MODIFICATION-change learning rate according to MaxUp paper
        if epoch < 100:
            lr = args.lr
        elif epoch < 150:
            lr = args.lr * 0.1
